      ...
      <div className="flex-1 relative bg-[#0a0e14]">
        ...
        <div className="absolute top-4 left-4 right-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {selectedEntity && (
              <Link
                to={`/hierarchy/case/${selectedEntity.id}`}
                className="holo-card px-4 py-2 flex items-center gap-2 hover:border-green-400/50 transition-all group"
              >
                <svg className="w-4 h-4 text-green-400 group-hover:text-green-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M12 3v18M3 9h18M3 15h18M9 3L3 9l6 6M15 3l6 6-6 6" />
                </svg>
                <span className="text-sm text-green-400 group-hover:text-green-300">Hierarchical View</span>
              </Link>
            )}
          </div>
        </div>
